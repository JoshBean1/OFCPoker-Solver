...
NO_FLUSHES = {3: NO_FLUSH_3, 5: NO_FLUSH_5, 6: NO_FLUSH_6, 7: NO_FLUSH_7}  # add support for 3 card hand
...
def evaluate_cards(*cards: Union[int, str, Card]) -> int:
    ...
    if not (TOP_ROW <= hand_size <= MAX_CARDS) or (hand_size not in NO_FLUSHES):
        raise ValueError(
            f"The number of cards must be {TOP_ROW} or between {MIN_CARDS} and {MAX_CARDS}."
